...
@Singleton
open class WithdrawalCallbackHandler(private val spaceParser: SpaceParser<Update, Chat>
    ...
    companion object{
        const val WITHDRAWAL = CallbackData.WITHDRAWAL
    }

    override fun getOrder() = Ordered.WITHDRAWAL

    override fun canHandle(bot: TelegramBotConfiguration?, input: Update): Boolean {
        println("---withdrawal callback")
        val match=  input.callbackQuery?.data?.matches(WITHDRAWAL.toRegex())
