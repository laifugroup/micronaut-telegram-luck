...
@Singleton
open class WaterReportCallbackHandler(private val spaceParser: SpaceParser<Update, Chat>
                                      , private val messageSource: MessageSource
                                      , private val luckCreditLogService: LuckCreditLogService,
    ) : TelegramHandler<AnswerCallbackQuery> {

    companion object{
        const val WATER_RATE = CallbackData.WATER_RATE
    }

    override fun getOrder() = Ordered.WATER_RATE

    override fun canHandle(bot: TelegramBotConfiguration?, input: Update): Boolean {
        println("---water_rate callback")
        val match=  input.callbackQuery?.data?.matches(WATER_RATE.toRegex())
    ...
    override fun handle(bot: TelegramBotConfiguration?, input: Update): Optional<AnswerCallbackQuery>{
        ...
        val it=luckCreditLogService.findWaterStatistics(input.callbackQuery?.from?.id?:0)
        ...
        val luckWaterReport = messageSource.getMessage( "luck.water.report", LocaleHelper.language(input), today,
            total,it.inviteSum?:BigDecimal.ZERO,it.childBoomRebateSum?:BigDecimal.ZERO).orElse(LocaleHelper.EMPTY)

        val answerCallbackQuery = AnswerCallbackQuery(input.callbackQuery.id, luckWaterReport, true, null, null)
